...
if _secret_from_env:
    ...
    _data_dir = (os.environ.get('AURALINKS_DATA_DIR') or ('/data' if os.path.isdir('/data') else os.path.join(os.path.dirname(__file__), 'data')))
...
# the bundle mount (fleet-bundle.onrender.com/auralinks) AND the bare custom domain
# (auralinks.de, where this app serves at root).
app.config['SESSION_COOKIE_NAME'] = 'auralinks_session'
...
# === SITE CONFIG ===
# Env names are AURALINKS_-prefixed: every bundle app shares ONE process env on
# Render, so unprefixed names (SITE_NAME, AFFILIATE_CODE...) would collide with
# the sibling sites' configs.
SITE_CONFIG = {
    'name': os.environ.get('AURALINKS_SITE_NAME', 'AuraLinks'),
    'name_part1': os.environ.get('AURALINKS_SITE_NAME_PART1', 'Aura'),
    'name_part2': os.environ.get('AURALINKS_SITE_NAME_PART2', 'Links'),
    'domain': os.environ.get('AURALINKS_SITE_DOMAIN', 'auralinks.de'),  # customer-facing domain (canonical/og/sitemap)
    # Litbuy is this client's featured/recommended agent (shows in SEO copy and
    # converter labels). agent_domain/agent_product_url stay on KakoBuy because
    # the catalogue's stored buy URLs are KakoBuy-wrapped and /go swaps the
    # affcode in place on that host (re-wrapping them in another agent would
    # nest agent URLs). The Litbuy default-pick happens in the agent picker,
    # where buy URLs are built from the raw seller link.
    'agent_name': os.environ.get('AURALINKS_AGENT_NAME', 'Litbuy'),
    'agent_domain': os.environ.get('AURALINKS_AGENT_DOMAIN', 'kakobuy.com'),
    'agent_signup_url': os.environ.get('AURALINKS_AGENT_SIGNUP_URL', 'https://litbuy.com/register?invitecode=vektor'),
    'agent_product_url': os.environ.get('AURALINKS_AGENT_PRODUCT_URL', 'https://www.kakobuy.com/item/details'),
    'affiliate_code': os.environ.get('AURALINKS_AFFILIATE_CODE', 'n62s8s'),
    'brand_color': os.environ.get('AURALINKS_BRAND_COLOR', '#ffd400'),
    'brand_color_shadow': os.environ.get('AURALINKS_BRAND_COLOR_SHADOW', '#e9c000'),
    'meta_pixel_id': os.environ.get('AURALINKS_META_PIXEL_ID', ''),
    'ga_measurement_id': os.environ.get('AURALINKS_GA_MEASUREMENT_ID', ''),
    'coupon_amount': os.environ.get('AURALINKS_COUPON_AMOUNT', '500'),
    'tagline': os.environ.get('AURALINKS_TAGLINE', 'A curated catalogue of 9,400+ finds. Updated daily.'),
    'product_count_label': os.environ.get('AURALINKS_PRODUCT_COUNT_LABEL', '9,400+'),
    'discord_url': os.environ.get('AURALINKS_DISCORD_URL', ''),
}
...
DATA_DIR = (os.environ.get('AURALINKS_DATA_DIR') or ('/data' if os.path.isdir('/data') else os.path.join(os.path.dirname(__file__), 'data')))
...
try:
    ...
    try:
        try:
            ...
            from database import get_db as _gdb
        _mc = _gdb()
        _own = SITE_CONFIG.get('affiliate_code', '')
        _n = 0
        for _foreign in ('bswes', 'ategoat'):
            _n += _mc.execute(
                "UPDATE products SET url = REPLACE(url, 'affcode=' || ?, 'affcode=' || ?) WHERE url LIKE '%affcode=' || ? || '%'",
                (_foreign, _own, _foreign)).rowcount
        _mc.commit(); _mc.close()
        if _n:
            print(f"Affcode scrub: rewrote {_n} product URLs (foreign -> {_own})")
    except Exception as _e:
...
def _b_litbuy(url, item_id, platform, code):
    ...
    same shape the client's spreadsheet links use). The old /transmit?url=
    endpoint is dead — Litbuy's app bounces it to the homepage."""
...
# Each agent carries an editable affiliate `code` (one code drives BOTH its buy
# URLs and its signup link) and a `signup` TEMPLATE with a {code} placeholder so
# editing the code in one place updates every link site-wide (master_admin agent
# editor). EXCEPTION: KakoBuy's signup is the client's ikako.vip short link with
# the code baked in — if his KakoBuy code ever changes, update that signup URL
# together with the code or signups will keep crediting the old code.
# Client (auralinks.de / Spready) sequence + invite codes — Litbuy is the
# recommended/featured agent and lists first; HubBuy replaced by Lovegobuy.
# These codes belong to THIS client only — never copy them to another site.
AGENTS = [
    {'key': 'litbuy',      'name': 'Litbuy',      'build': _b_litbuy,      'color': '#ffc400', 'domain': 'litbuy.com',      'code': 'vektor',                    'signup': 'https://litbuy.com/register?invitecode={code}',     'coupon': 'Up to $150 in coupons', 'recommended': True},
    ...
    {'key': 'kakobuy',     'name': 'KakoBuy',     'build': _b_kakobuy,     'color': '#0d9488', 'domain': 'kakobuy.com',     'code': SITE_CONFIG.get('affiliate_code', 'n62s8s'), 'signup': 'https://ikako.vip/r/n62s8s',           'coupon': 'Up to $500 shipping credit'},
...
> The #1 {s['agent_name']} finds website. Browse curated products at the best prices. Sign up to any agent for up to ${s['coupon_amount']} in welcome coupons.
...
def _settings_defaults():
    ...
    return {
        ...
        'hero_sub': (str(SITE_CONFIG.get('product_count_label', '')) + ' curated products. Open every listing with the agent you trust — Litbuy, Hipobuy, Mulebuy, USFANS, Oopbuy, KakoBuy, ACBuy, JoyaGoo, Sugargoo, or Lovegobuy.').strip(),
        ...
        'meta_description': name + ' — agent-neutral catalogue of curated finds. Open every listing on Litbuy, Hipobuy, Mulebuy, USFANS, Oopbuy, KakoBuy, ACBuy, JoyaGoo, Sugargoo, or Lovegobuy.',
    }
