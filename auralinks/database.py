...
# Per-site isolated storage. In the bundle, wsgi.py sets AURALINKS_DATA_DIR to
# the persistent disk subdir (/var/data/auralinks on Render); standalone dev
# falls back to ./data next to this file. NEVER a CWD-relative path — that
# would collide with sibling sites running in the same process.
_DATA_DIR = (os.environ.get('AURALINKS_DATA_DIR')
             or ('/data' if os.path.isdir('/data') else os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data')))
DB_PATH = os.path.join(_DATA_DIR, 'site.db')
