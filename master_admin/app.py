...
@app.route('/products/<site_id>/move-to-position', methods=['POST'])
def products_move_to_position(site_id):
    """Proxy 'place product at exact position N (in a category/view)' to the site."""
    site = next((s for s in load_sites() if s['id'] == site_id), None)
    if not site:
        return jsonify({'ok': False, 'error': 'site not found'}), 404
    body = request.get_json(silent=True) or {}
    data, err = _call_site_detailed(site, '/admin/products/move-to-position', method='POST', json_body=body)
    if err:
        return jsonify({'ok': False, 'error': err}), 502
    return jsonify(data or {'ok': True})
