...
@app.route('/qc-finder')
def qc_finder():
    return render_template('qc_finder.html')


@app.route('/api/qc-finder')
def api_qc_finder():
    """Aggregate every QC + gallery photo we can find for a pasted product URL.

    Sources (in order):
      1. ategoat's QC API for the matching listing
      2. The product gallery on ategoat's product page (geilicdn images)
      3. The seller's Weidian gallery (detail.weidian.com HTML)

    Cached on the persistent disk so repeat lookups for the same URL are
    instant.
    """
    url = (request.args.get('url') or '').strip()
    if not url:
        return jsonify({'error': 'Not a valid URL', 'photos': []})
    raw = _unwrap_agent_url(url)
    platform, item_id = _parse_item_url(raw)
    if not item_id:
        return jsonify({'error': 'Not a valid URL', 'photos': []})

    cache_key = f'qcfinder:{platform}:{item_id}'
    cached = cache_get(cache_key, max_age_seconds=7 * 24 * 3600)
        ...
        if items:
            ...
            try:
                q = _r.get('https://www.ategoat.com/wp-json/wiligoods/v1/qc/list',
                           params={'product_id': aid, 'limit': 200},
                           headers={'User-Agent': 'Mozilla/5.0', 'Accept': 'application/json'},
                           timeout=6)
                for it in (((q.json() or {}).get('data') or {}).get('items') or []):
                    add(it.get('image') or it.get('url'), 'ategoat-qc')
            except Exception:
                pass
                ...
                if r.status_code == 200:
                    ...
                    for u in img_re.findall(r.text):
                        if any(skip in u for skip in ('hz_img_', 'icon-', '/avatar', 'login_', 'wd_logo', 'common-')):
                            continue
                        add(u, 'ategoat-gallery')
            except Exception:
                pass
    except Exception:
        pass

    # Source 3: Weidian seller gallery (only for Weidian-hosted items)
    if platform == 'weidian':
        try:
            r = _r.get(f'https://detail.weidian.com/item.html?itemId={item_id}',
                       headers={'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1'},
                       timeout=8)
            if r.status_code == 200:
                img_re = _re.compile(r'https?://(?:si|sd)\.geilicdn\.com/[a-zA-Z0-9_/.\-]+\.(?:jpg|jpeg|png|webp)', _re.I)
                for u in img_re.findall(r.text):
                    if any(skip in u for skip in ('hz_img_', 'pccover', 'icon-', '/avatar', 'login_', 'wd_logo', 'common-')):
                        continue
                    add(u, 'weidian-gallery')
        except Exception:
            pass
